...
an object used by an implementation to implement the requirements of this \documenttypename{}
\enternote
By its definition a graphics resource is an implementation detail. Typically it will be something like a graphics device, a render target, or some other graphics subsystem object. However it is also perfectly acceptable for it to be, e.g., an aggregate composed of multiple graphics subsystem objects which, together, allow the implementation to provide the required functionality.

This \documenttypename{} only imposes requirements involving graphics resources in a few instances. Where it does so, it only specifies that certain behavior is required and makes no imposition on how the required behavior should be implemented.

A graphics resource is only directly accessible if an implementation chooses to expose it by providing a native handle (\ref{\iotwod.req.native}).
\exitnote
...
\indexdefn{native path geometry}
\definition{native path geometry}{\iotwod.general.defns.nativepathgeometry}
a graphics resource representing a path geometry
