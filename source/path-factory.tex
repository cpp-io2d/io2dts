...
group data from which \tcode{path} objects are created.

\pnum
A \tcode{path_factory} is a contiguous container. (See [container.requirements.general] in \cppseventeen.) This means that the consecutive path geometries within its path group data and the path segments contained in each of those path geometries are contiguous. As such the iterators for a \tcode{path_geometry} object present the path group data as a contiguous series of path segments.

\pnum
The type of a path segment in a \tcode{path_factory} object is  \tcode{path_factory::path_data}.

%\pnum
%Any manipulation of the path group data which results in path group data that cannot be processed in the manner specified in \ref{pathgeometries.processing} shall result in undefined behavior.
%
...
\begin{itemdescr}
	\pnum
	\effects
	\tcode{_Data.emplace_back(path_factory::path_new_sub_path())}.
...
\begin{itemdescr}
	...
	\begin{itemize}
	\item \tcode{_Data.emplace_back(path_factory::path_close_path())}.
...
\begin{itemdescr}
	...
	\pnum
	\tcode{_Data.emplace_back(path_factory::path_curve_to(pt0, pt1, pt2))}.
...
\begin{itemdescr}
	\pnum
	\effects
	\tcode{_Data.emplace_back(path_factory::path_line_to(pt))}.
...
\begin{itemdescr}
	\pnum
	\effects
	\tcode{_Data.emplace_back(path_factory::path_move_to(pt))}.
...
\begin{itemdescr}
	\pnum
	\effects
	\tcode{_Data.emplace_back(path_factory::path_rel_curve_to(dpt0, dpt1, dpt2))}.
...
\begin{itemdescr}
	\pnum
	\effects
	\tcode{_Data.emplace_back(path_factory::path_rel_line_to(pt))}.
...
\begin{itemdescr}
	\pnum
	\effects
	\tcode{_Data.emplace_back(path_factory::path_rel_move_to(dpt))}.
...
\begin{itemdescr}
	\pnum
	\effects
	\tcode{_Data.emplace_back(path_factory::path_change_matrix(m))}.
...
\begin{itemdescr}
	\pnum
	\effects
	\tcode{_Data.emplace_back(path_factory::path_change_origin(pt)))}.
