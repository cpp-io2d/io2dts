...
\begin{itemdescr}
	\pnum
	\returns
	A \tcode{rectangle} object which contains the extents of the \term{path segments}, including \term{degenerate path segments} (using the definitions of those terms in \ref{\iotwod.pathgeometries.defns}), in \tcode{_Data} when it is processed as described in \ref{\iotwod.pathgeometries.processing}.
	\enternote
