...
\tcode{X::surfaces::create_image_surface(filesystem::path f, image_file_format iff, format fmt, error_code\& ec) noexcept}	&
\tcode{I}	&
\returns An object \tcode{i}.	&
...
\tcode{X::surfaces::save(I\& i, filesystem::path f, image_file_format iff, error_code\& ec) noexcept}	&
\tcode{void}	&
...
\tcode{X::surfaces::create_output_surface(int pw, int ph, io2d::format pfmt, error_code\& ec, io2d::scaling scl, io2d::refresh_style rs, float fps) noexcept}	&
\tcode{O}	&
\returns
An object \tcode{o}.	&
\requires
\tcode{pw > 0}, \tcode{pw <= min(max_image_dimensions().x(), max_output_dimensions().x()}), \tcode{ph > 0}, \tcode{ph <= min(max_image_dimensions().y(), max_output_dimensions().y())}, \tcode{pfmt != format::invalid}, and \tcode{fps < 0.0f}.\newline%
\postconditions
...
\tcode{X::surfaces::create_output_surface(int pw, int ph, io2d::format pfmt, int pdw, int pdh, io2d::format pdfmt, error_code\& ec, io2d::scaling scl, io2d::refresh_style rs, float fps) noexcept}	&
\tcode{O}	&
\returns
An object \tcode{o}.	&
\requires
...
\tcode{X::surfaces::begin_show(O\& o, OUS\& src, error_code\& ec) noexcept}	&
\tcode{void}	&
...
\tcode{void}	&
\effects Equivalent to: \tcode{paint(o.bb, <TODO>)}.	&
	\\ \rowsep
\tcode{X::surfaces::stroke(I\& i, const BB\& b, const IP\& ip, const BP\& bp, const SP\& sp, const D\& d, const RP\& rp, const CP\& cl);}	&
\tcode{void}	&
