...
\pnum
\begin{note}
Brushes are immutable objects. As such, the contents of the class type for which \tcode{X::brushes::brush_data_type} is an identifier are able to be highly tailored to the \underlyingrendandpresenttechs which are used by \tcode{X}. Indeed, while it is required to report its \tcode{brush_type} by one of the member functions below, it need not even store a variable of that type so long as it can correctly provide it when that member function is called.
\end{note}
