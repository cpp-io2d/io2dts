...
\pnum
This section is normative. It describes how to convert the path group of a properly formed \tcode{path_factory} object from a collection of \tcode{path_data::data} objects to a collection of \tcode{path_data::data} objects which have had their points transformed in accordance with the origin and transformation matrix of the \tcode{path_factory} object and any \tcode{path_data::change_origin} and \tcode{path_data::change_matrix} objects in the path group of the \tcode{path_factory} object.

\pnum
The following code shows how to properly process a \tcode{path_factory} object \tcode{p} and store the results into a \tcode{vector<path_data::data>}:
