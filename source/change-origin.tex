...
The class \tcode{path_factory::path_change_origin} describes an operation on a path group.

\pnum
This operation changes the origin point for a path group to be the value returned by \tcode{*this.origin()}. As shown in \ref{pathgeometries.processing}, the new origin point does not affect any operations that came before this operation. It is only used in processing operations that come after it. It continues to be used until another \tcode{path_factory::path_change_origin} object is encountered or the end of the path group is reached.
