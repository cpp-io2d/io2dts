...
\pnum
\begin{note}
The \tcode{image_surface} class intentionally does not provide copy semantics because with many modern graphics technologies, making such a copy is almost always a very time consuming operation. This function allows users to make a copy of an \tcode{image_surface} object while preserving the move-only semantics of the \tcode{image_surface} class.
\end{note}
\end{itemdescr}
...
\pnum
\requires
\tcode{rad.x()} and \tcode{rad.y()} are both greater than \tcode{0.0}.
