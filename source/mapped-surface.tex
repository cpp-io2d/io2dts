...
\pnum
A \tcode{mapped_surface} object can only be created by the \tcode{image_surface::map} function. Creation of a \tcode{mapped_surface} object fails if the format of the pixel data would be \tcode{format::invalid} or \tcode{format::unknown}.

\pnum
The pixel data is presented as an array in the form of a pointer to (possibly \tcode{const}) \tcode{unsigned char}.

\pnum
The actual format of the pixel data depends on the \tcode{format} enumerator returned by calling \tcode{mapped_surface::format} and is native-endian. For more information, see the description of the \tcode{format} enum class (\ref{\iotwod.format}).
...
\pnum
Users shall call \tcode{mapped_surface::commit_changes} to commit changes made to the surface's data prior to the destruction of the \tcode{mapped_surface} object.
\end{itemdescr}
...
\pnum
\errors
The errors, if any, produced by this function are \impldefplain{mapped_surface!commit_changes}.
\end{itemdescr}
...
\tcode{experimental::io2d::format::invalid}.
\end{itemdescr}
...
\pnum
\remarks
This function shall return the value \tcode{0} if \tcode{this->format() == \\
experimental::io2d::format::unknown || this->format() == \\
