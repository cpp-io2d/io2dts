...
determined if it is outside the boundary of the source \tcode{pattern} or 
\tcode{surface} during a call to \tcode{surface::fill}, \tcode{surface::mask}, 
\tcode{surface::paint}, or \tcode{stroke}. See 
Table~\ref{tab:\iotwod.extend.meanings} for the meaning of each \tcode{extend} 
enumerator.
...
\begin{libreqtab2}
 ...
 \endhead
 \tcode{none}
 & Any pixel outside of the bounds of the \tcode{pattern} or \tcode{surface} 
 which must be set will be set to transparent black.
 \\
 \tcode{repeat}
 & A pixel outside of the bounds of the \tcode{pattern} or \tcode{surface} 
 which must be set will be set to the color it would have been set to if the 
 \tcode{pattern} or \tcode{surface} was infinitely large and repeated itself in 
