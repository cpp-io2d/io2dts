...
\pnum
%In a conforming implementation, \tcode{numeric_limits<float>::is_iec559} evaluates to \tcode{true}.
Certain requirements of this \documenttypename{} assume that 
\tcode{numeric_limits<float>::is_iec559} evaluates to \tcode{true}.

\pnum
If \tcode{numeric_limits<float>::is_iec559} evaluates to \tcode{false}, the 
observable behavior of a well-formed program is \unspecnorm.
\begin{note}
An implementation in which \tcode{numeric_limits<float>::is_iec559} evaluates 
to \tcode{false} should comply with the requirements of this \documenttypename 
subject to any limitations resulting from 
\tcode{numeric_limits<float>::is_iec559} evaluating to \tcode{false}.
\end{note}
%The 
%behavior of these requirements is \impldefplain{\tcode{numeric_limits<float>::is_iec559} evaluates to \tcode{false}} 
%if \tcode{numeric_limits<float>::is_iec559} evaluates to \tcode{false}.
