%!TEX root = io2d.tex
\rSec0 [refreshrate] {Enum class \tcode{refresh_rate}}

\rSec1 [refreshrate.summary] {\tcode{refresh_rate} Summary}

\pnum
The \tcode{refresh_rate} enum class describes when the Draw Callback (Table~\ref{tab:displaysurface.state.listing}) of a \tcode{display_surface} object shall be called. See Table~\ref{tab:refreshrate.meanings} for the meaning of each \tcode{} enumerator.

\rSec1 [refreshrate.synopsis] {\tcode{refresh_rate} Synopsis}
...
\rSec1 [refreshrate.enumerators] {\tcode{refresh_rate} Enumerators}
