...
\pnum
A linear gradient has a \term{begin point} and an \term{end point}, each of which are objects of type \tcode{basic_point_2d}.

\pnum
A linear gradient for which the distance between its begin point and its end point is <TODO>\tcode{basic_point_2d::zero()} is a \term{degenerate linear gradient}.
...
\pnum
\begin{example}
Given a linear gradient with a begin point of <TODO>\tcode{basic_point_2d(0.0f, 0.0f)} and an end point of <TODO>\tcode{basic_point_2d(10.0f, 5.0f)}, a gradient stop offset value of 0.6f would correspond to the point <TODO>\tcode{basic_point_2d(6.0f, 3.0f)}.
\end{example}
...
\pnum
A radial gradient has a \term{start circle} and an \term{end circle}, each of which is defined by a \tcode{basic_circle} object.
...
\item the distance between the center point of its start circle and the center point of its end circle is <TODO>\tcode{basic_point_2d::zero()}; or,
