...
\pnum
In addition to brushes (\ref{\iotwod.surface.rendering.brushes}), all rendering and composing operation except for painting use a \term{source path} of type \tcode{path_group}.
...
\item If the Dash Pattern has its default value or if its \tcode{vector<float>} member is empty, the path segments shall be rendered as a continuous path.
...
\begin{example}
If an open path consisting solely of a vertical line from \tcode{vector_2d(20.0f, 20.0f)} to \tcode{vector_2d(20.0f, 60.0f)} is to be composed to the \underlyingsurface, the Line Cap is \tcode{line_cap::none}, the Line Width is \tcode{12.0f}, and the Transformation Matrix is \tcode{matrix_2d::init_scale(0.5f, 1.0f)}, then the line will end up being composed within the area \tcode{rectangle( \{ 7.0f, 20.0f \}, \{ 13.0f, 60.0f \} )} on the \underlyingsurface. The Transformation Matrix causes the center of the \xaxis of the line to move from \tcode{20.0f} to \tcode{10.0f} and then causes the horizontal width of the line to be reduced from \tcode{12.0f} to \tcode{6.0f}.
\end{example}
