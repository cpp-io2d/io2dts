...
\pnum
The \term{Brush Coordinate Space} is the standard coordinate space of the Source Brush (\ref{surface.rendering.brushes}). Its transformation matrix is the Brush Properties' Brush Matrix (\ref{\iotwod.brushprops.summary}).
...
\item Using the Source Path (\ref{surface.rendering.sourcepath}) and the Fill Rule (\ref{\iotwod.brushprops.summary}), determine whether $up$ shall be filled; if so, proceed with the remaining steps.
